...
#ifndef DOMAIN_BRIDGE__WAIT_FOR_SUBSCRIPTION_HPP_
#define DOMAIN_BRIDGE__WAIT_FOR_SUBSCRIPTION_HPP_
...
/// Wait for a subscription to be available (or not)
/*
 * \return true if the wait was successful, false if a timeout occured.
 */
inline bool wait_for_subscription(
  ...
  std::chrono::nanoseconds sleep_period = std::chrono::milliseconds(100))
{
  ...
  auto predicate = [&node, &topic_name]() -> bool {
      // A subscription is available if the count is greater than 0
...
#endif  // DOMAIN_BRIDGE__WAIT_FOR_SUBSCRIPTION_HPP_
